# -*- coding: utf-8 -*-
"""
Base de Datos SQLite - Sucursal Veraguas
...
class VeraguasDB:
    ...
    def connect(cls):
        try:
            db_path = Config.SQLITE_VERAGUAS
            os.makedirs(os.path.dirname(db_path), exist_ok=True)
            cls._connection = sqlite3.connect(db_path, check_same_thread=False)
            cls._connection.row_factory = sqlite3.Row
            cls._create_tables()
            logger.info("✓ Conexión a SQLite Veraguas establecida")
            return cls._connection
        except Exception as e:
            logger.error(f"✗ Error al conectar a SQLite Veraguas: {e}")
            return None
    ...
    def _create_tables(cls):
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS cotizaciones (
                ...
                total REAL NOT NULL,
                sucursal TEXT DEFAULT 'VERAGUAS',
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                pdf_path TEXT,
        ...
        logger.info("✓ Tablas creadas en Veraguas")
...
def get_veraguas_db():
    return VeraguasDB.get_connection()