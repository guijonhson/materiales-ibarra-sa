# -*- coding: utf-8 -*-
"""
Base de Datos SQLite - Sucursal Chitré
...
class ChitréDB:
    ...
    def connect(cls):
        try:
            db_path = Config.SQLITE_CHITRE
            os.makedirs(os.path.dirname(db_path), exist_ok=True)
            cls._connection = sqlite3.connect(db_path, check_same_thread=False)
            cls._connection.row_factory = sqlite3.Row
            cls._create_tables()
            logger.info("✓ Conexión a SQLite Chitré establecida")
            return cls._connection
        except Exception as e:
            logger.error(f"✗ Error al conectar a SQLite Chitré: {e}")
            return None
    ...
    def _create_tables(cls):
        ...
        logger.info("✓ Tablas creadas en Chitré")
...
def get_chitre_db():
    return ChitréDB.get_connection()